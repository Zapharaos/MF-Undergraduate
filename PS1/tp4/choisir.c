/* bonjour, je n'ai pas trouve comment fermer les autres processus
 * j'ai essaye d'utiliser un tube mais sans succes
 * donc j'ai envoye un dernier message avant de terminer le vainqueur
 * je m'excuse, bonne journée a vous
*/
...
void treat_process(int N, int index, int (*child)[2], int (*parent)[2], int (*way)[2])
{
    ...
    while(loop)
    {
        ...
        if(value == last_value)
        {
            ...
            // sends "end of election" value to other process
...
void choose(int N)
{
    ...
    // write : initial way to first pipe
    write_to_pipe(way, 0, 1);

    // write : initial value to first pipe
