 ...
 * *******************************/
int cast_string_to_int(char *arg)
{
    // variables
    char *endptr, *str;
    str = arg;
    ...
    // if : not found
    if (endptr == str)
        raler("cast_string_to_int nothing found");
...
int read_from_pipe(int (*pipe)[2], int index)
{
    ...
    // cast : string to int
    return cast_string_to_int(buffer);
}
...
void handle_winner(int index)
{
    ...
    /* uncomment to call huisier.c from choisir.c */
...
void treat_process(int N, int index, int (*child)[2], int (*parent)[2], int (*way)[2])
{
    ...
    while(loop)
    {
        ...
        if(value == last_value)
        {
            ...
            // write : value (depending on the way)
            if(is_forward)
                write_to_pipe(child, next, getpid());
            else
                write_to_pipe(parent, prev, getpid());
        ...
        switch(value)
        {
            ...
            case 0: // eliminated
                ...
                is_alive = false;
                break;
            default: // update : value (depending on the way)
                value--;
                break;
        }
...
void choose(int N)
{
    ...
    // write : initial way
    write_to_pipe(way, 0, 1);

    // write : initial value
    write_to_pipe(child, 0, value);
...
int main (int argc, char **argv)
{
    ...
    // cast : argv[1] to int
    int N = cast_string_to_int(argv[1]);
    choose(N);
